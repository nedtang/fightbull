...
##        return player_cards
    ...
    def the_bull(self,num):
        ...
        for y in num:
            ...
            max_card.append(self.poker[y][1])
            
            List_card.append(self.poker[y][2])
...
#单副牌内
        ...
        clean_cards=list(range(1,53))

        for o in range(3):

            player_card=random.sample(clean_cards,5*3)

            for c in player_card:
                
                clean_cards.remove(c)
                
            player_cards=[]
            
            for z in range(3):

                exec("play"+str(z)+"="+"player_card[5*z:5*(z+1)]")

                player_cards.append(eval("play"+str(z)))


            for i in range(len(player_cards)):

                exec("play"+str(i)+"=b.the_bull(player_cards[i])")
